...
undefined){if(b.inArray(a,o)>-1){o=b.grep(o,function(C){return C!=a});b("#"+c.attr("id")).trigger("fvmapSectionDeselect",[g[a].pic+"?size="+f.ssize,g[a].name,a])}else if(!(parseFloat(g[a].minPrice)>parseFloat(v)||parseFloat(g[a].maxPrice)<parseFloat(u))){o=[a].concat(o);if(f.interactWithTicketList){var r=g[a].sections;b("#"+c.attr("id")).trigger("fvmapSectionSelect",[g[a].pic+"?size="+f.ssize,g[a].name,r])}else b("#"+c.attr("id")).trigger("fvmapSectionSelect",[g[a].pic+"?size="+f.ssize,g[a].name,
...
s.split("**"),d=p(s.split("**")[0]),h=0;if(e.length>1)h=parseFloat(s.split("**")[1]);k[d]=c[s];if(g[k[d]]!=undefined){g[k[d]].section=d;for(e=0;e<f.ticketList.items.length;e++){var m=f.ticketList.items[e];if(m.section.toLowerCase()==d.toLowerCase())if(g[k[d]].sections==undefined){g[k[d]].sections=[m];g[k[d]].ids=[m.id]}else if(b.inArray(m.id,g[k[d]].ids)<0){g[k[d]].sections.push(m);g[k[d]].ids.push(m.id)}}if(g[k[d]].minPrice==undefined){g[k[d]].minPrice=h;g[k[d]].maxPrice=h}else{if(h<g[k[d]].minPrice)g[k[d]].minPrice=
h;if(h>g[k[d]].maxPrice)g[k[d]].maxPrice=h}}}d=b(this);if(d.find(".section.focus").css("color")!=null)I=G(d.find(".section.focus").css("color"));if(d.find(".section.focus").css("opacity")!=null)J=parseFloat(d.find(".section.focus").css("opacity"));if(d.find(".section.available").css("color")!=null)y=G(d.find(".section.available").css("color"));if(d.find(".section.available").css("opacity")!=null)z=parseFloat(d.find(".section.available").css("opacity"));if(d.find(".section.selected").css("color")!=
null)E=G(d.find(".section.selected").css("color"));if(d.find(".section.selected").css("opacity")!=null)F=parseFloat(d.find(".section.selected").css("opacity"));for(id in g)Q(id,this);f.priceFilter&&K(d);d.trigger("fvmapReady")}}}})(jQuery);
